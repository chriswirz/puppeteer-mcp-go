...
// TestScreenshotPruning covers the retention limits: off unless configured,
// oldest deleted first, and the capture just written always kept.
func TestScreenshotPruning(t *testing.T) {
	write := func(t *testing.T, cfg Config) (*Server, func(name string, size int, age time.Duration) string) {
		t.Helper()
		srv := &Server{cfg: cfg}
		if err := os.MkdirAll(cfg.Browser.ScreenshotDir, 0o755); err != nil {
			t.Fatal(err)
		}
		return srv, func(name string, size int, age time.Duration) string {
			path := filepath.Join(cfg.Browser.ScreenshotDir, name)
	...
	t.Run("off by default", func(t *testing.T) {
		...
		if cfg.Browser.KeepScreenshots != 0 || cfg.Browser.MaxScreenshotDirBytes != 0 {
			t.Fatalf("pruning defaults = %d/%d, want 0/0: it must not delete anything unasked",
				cfg.Browser.KeepScreenshots, cfg.Browser.MaxScreenshotDirBytes)
		}
		srv, add := write(t, cfg)
		for _, n := range []string{"old.png", "mid.png"} {
			add(n, 10, time.Hour)
		}
		newest := add("new.png", 10, 0)
		srv.pruneScreenshots(newest)
		if got := names(t, cfg.Browser.ScreenshotDir); len(got) != 3 {
			t.Errorf("files = %v, want all three kept", got)
		}
	})

	t.Run("count limit deletes oldest first", func(t *testing.T) {
		cfg := DefaultConfig()
		if err := cfg.Normalize(t.TempDir()); err != nil {
			t.Fatal(err)
		}
		cfg.Browser.KeepScreenshots = 2
		srv, add := write(t, cfg)
		add("a-oldest.png", 10, 3*time.Hour)
		add("b-older.png", 10, 2*time.Hour)
		add("c-recent.png", 10, time.Hour)
		newest := add("d-newest.png", 10, 0)
		srv.pruneScreenshots(newest)
		got := names(t, cfg.Browser.ScreenshotDir)
		want := []string{"c-recent.png", "d-newest.png"}
		if strings.Join(got, ",") != strings.Join(want, ",") {
			t.Errorf("files = %v, want %v", got, want)
		}
	})

	t.Run("byte limit keeps the new capture even when it alone exceeds it", func(t *testing.T) {
		cfg := DefaultConfig()
		if err := cfg.Normalize(t.TempDir()); err != nil {
			t.Fatal(err)
		}
		cfg.Browser.MaxScreenshotDirBytes = 100
		srv, add := write(t, cfg)
		add("old.png", 60, time.Hour)
		newest := add("new.png", 500, 0)
		srv.pruneScreenshots(newest)
		got := names(t, cfg.Browser.ScreenshotDir)
		if strings.Join(got, ",") != "new.png" {
			t.Errorf("files = %v, want only new.png: the capture just written is never deleted", got)
		}
	})

	t.Run("non-png files are left alone", func(t *testing.T) {
		cfg := DefaultConfig()
		if err := cfg.Normalize(t.TempDir()); err != nil {
			t.Fatal(err)
		}
		cfg.Browser.KeepScreenshots = 1
		srv, add := write(t, cfg)
		add("notes.txt", 10, time.Hour)
		add("old.png", 10, time.Hour)
		newest := add("new.png", 10, 0)
		srv.pruneScreenshots(newest)
		got := names(t, cfg.Browser.ScreenshotDir)
		want := []string{"new.png", "notes.txt"}
		if strings.Join(got, ",") != strings.Join(want, ",") {
			t.Errorf("files = %v, want %v", got, want)
		}
	})
...
func TestConfigRejectsNegativeScreenshotLimits(t *testing.T) {
	cfg := DefaultConfig()
	cfg.Browser.KeepScreenshots = -1
	if err := cfg.Normalize(t.TempDir()); err == nil || !strings.Contains(err.Error(), "keep_screenshots") {
		t.Fatalf("Normalize = %v, want an error naming keep_screenshots", err)
	}
}
