...
func (b *Browser) startLocked() error {
	...
	// Ensure Playwright drivers are available before initializing
	if err := ensureDriversDownloaded(); err != nil {
		b.logger.Printf("warning: could not auto-download drivers: %v", err)
		// Continue anyway - user might have them installed manually
	}
	...
	if err != nil {
		var msg string
		if b.cfg.CDPURL != "" {
			// In attach mode, the user has CDP configured - give them relevant advice
			msg = fmt.Sprintf("starting playwright (you are using CDP mode with %s): "+
				"the browser drivers must be installed. Run `puppeteer-mcp --install-drivers` or "+
				"`go run github.com/playwright-community/playwright-go/cmd/playwright@latest install --with-deps chromium`. "+
				"Verify the CDP server is accessible at %s", b.cfg.CDPURL, b.cfg.CDPURL)
		} else {
			// In launch mode, suggest installing drivers or using an existing Chrome
			msg = "starting playwright: run `puppeteer-mcp --install-drivers` once, " +
				"or set browser.channel to a Chrome you already have"
		}
...
func (b *Browser) attachLocked() error {
	// Launch the browser if a launch command is configured
	if b.cfg.CDPLaunchCmd != "" {
		b.logger.Printf("launching browser: %s", b.cfg.CDPLaunchCmd)
		cmd := exec.Command("sh", "-c", b.cfg.CDPLaunchCmd)
		cmd.Stdout = os.Stderr
		cmd.Stderr = os.Stderr
		if err := cmd.Start(); err != nil {
			return fmt.Errorf("failed to launch browser: %w", err)
		}
		// Give the browser a moment to start and open the debugging port
