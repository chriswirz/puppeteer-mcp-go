...
func (s *Server) writeScreenshot(path string, data []byte) (string, error) {
	...
	s.pruneScreenshots(path)
	return path, nil
}

// pruneScreenshots enforces the configured limits on the screenshot directory,
// deleting oldest first. keep is the capture just written, which survives
// whatever the limits say - returning a path to a file this call had already
// deleted would be worse than exceeding the cap by one.
//
// Failures are ignored deliberately: the screenshot itself succeeded, and a
// directory that could not be tidied is not a reason to fail the tool call.
func (s *Server) pruneScreenshots(keep string) {
	max, maxBytes := s.cfg.Browser.KeepScreenshots, s.cfg.Browser.MaxScreenshotDirBytes
	if max <= 0 && maxBytes <= 0 {
		return
	}
	dir := s.cfg.Browser.ScreenshotDir
	...
	for _, e := range entries {
		...
		if sameFile(path, keep) {
			continue
		}
		shots = append(shots, shot{path: path, size: info.Size(), mod: info.ModTime()})
	}
	// The kept capture counts against both limits but is never a candidate, so
	// the count below is of everything in the directory.
	count := len(shots) + 1
	sort.Slice(shots, func(i, j int) bool { return shots[i].mod.Before(shots[j].mod) })
	for _, sh := range shots {
		overCount := max > 0 && count > max
...
// sameFile reports whether two paths name the same capture. Both are built
// from the same directory join, so comparing the cleaned paths is enough -
// case-insensitively where the filesystem is.
