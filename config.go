...
type BrowserConfig struct {
	...
	// CDPLaunchCmd, when set, is a shell command to launch the browser for
	// attach mode. The command is executed before connecting over CDP. This
	// allows automatic browser startup instead of requiring manual launch.
	// Example: `C:\Program Files\Google\Chrome\Application\chrome.exe --remote-debugging-port=9222 --user-data-dir=D:\chrome-debug`
	CDPLaunchCmd string `json:"cdp_launch_cmd"`
	...
	// KeepScreenshots and MaxScreenshotDirBytes prune the screenshot directory
	// after each capture, deleting the oldest files until both limits hold: a
	// file count and a total size in bytes. Zero - the default for both - never
	// deletes anything, because the captures are the operator's files and a
	// server that quietly removes them is a bad surprise.
	//
	// Pruning only ever considers the .png files directly in the directory, and
	// never the capture just written.
	KeepScreenshots       int   `json:"keep_screenshots"`
	MaxScreenshotDirBytes int64 `json:"max_screenshot_dir_bytes"`
...
func LoadConfig(path string, explicit bool) (Config, error) {
	...
	// Always attempt to normalize the data to UTF-8 Unix format, regardless of
	// whether the conversion succeeds or fails. This handles different encodings
	// and line endings from various editors and platforms.
	data = normalizeConfigData(data)

	dec := json.NewDecoder(bytes.NewReader(data))
	// Allow unknown fields so extra config items don't cause errors
...
// normalizeConfigData converts config data to UTF-8 Unix format (LF line endings).
// It always attempts conversion but continues with best-effort results even if
// conversion partially fails. This handles configs created by different editors,
// platforms, and shells (including PowerShell redirection).
func normalizeConfigData(data []byte) []byte {
	// Remove UTF-8 BOM if present (PowerShell redirection adds this)
	data = bytes.TrimPrefix(data, []byte{0xEF, 0xBB, 0xBF})

	// Convert Windows line endings (CRLF) to Unix (LF)
	data = bytes.ReplaceAll(data, []byte{'\r', '\n'}, []byte{'\n'})

	// Convert old Mac line endings (CR) to Unix (LF)
	// Only if they're not already part of CRLF (which we just handled)
...
func (c *Config) Normalize(dir string) error {
	...
	if b.KeepScreenshots < 0 || b.MaxScreenshotDirBytes < 0 {
		return fmt.Errorf("browser.keep_screenshots/max_screenshot_dir_bytes: must not be negative; 0 turns pruning off")
	}
