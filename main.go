...
The browser drivers are downloaded automatically on first run if needed (one-time).
You can also pre-download them with --install-drivers.
...
Flags:
  -c, --config <path>      configuration file to read (default: config.json in
      ...
      --cdp-launch-cmd <cmd>
                           command to launch the browser for attach mode
                           (runs before connecting over CDP)
  ...
  -u, --url <url>          full URL clients connect to over http, and what the
      ...
      --install-drivers    install Playwright browser drivers and exit
                           (one-time setup required on each system)
...
Examples:
  puppeteer-mcp --install-drivers       # one-time setup
  puppeteer-mcp --update                # update to latest release
  puppeteer-mcp
...
func parseFlags(args []string) (*options, error) {
	...
	str(&opts.cdp, "", "cdp")
	str(&opts.cdpLaunchCmd, "", "cdp-launch-cmd")
	...
	boolean(&opts.installDrivers, "install-drivers")
	boolean(&opts.update, "update")
...
func applyOptions(cfg *Config, opts *options) {
	...
	if opts.cdpLaunchCmd != "" {
		cfg.Browser.CDPLaunchCmd = opts.cdpLaunchCmd
	}
	if opts.interactive {
...
// installDrivers initializes Playwright and installs browser drivers.
func installDrivers() error {
	fmt.Fprintf(os.Stderr, "Installing Playwright browser drivers...\n")
	pw, err := playwright.Run()
	if err != nil {
		return fmt.Errorf("failed to install drivers: %w", err)
	}
	defer pw.Stop()
	fmt.Fprintf(os.Stderr, "Browser drivers installed successfully.\n")
	return nil
}

// updateExecutable downloads and installs the latest release from GitHub.
func updateExecutable() error {
	...
	resp, err := http.Get("https://api.github.com/repos/chriswirz/puppeteer-mcp-go/releases/latest")
	if err != nil {
	...
	switch runtime.GOOS {
	case "windows":
		binaryName = "puppeteer-mcp-windows-" + runtime.GOARCH + ".exe"
	case "linux":
		binaryName = "puppeteer-mcp-linux-" + runtime.GOARCH
	case "darwin":
		binaryName = "puppeteer-mcp-darwin-" + runtime.GOARCH
	default:
	...
	fmt.Fprintf(os.Stderr, "Found %s. Downloading...\n", release.TagName)
	...
	// On Windows, we need to make the file executable
	// On Unix, preserve existing permissions
