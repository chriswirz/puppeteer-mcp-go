...
with binaries for Linux, Windows and macOS on amd64 and arm64. Versions are `v0.1.<build number>`.
...
It drives the browser through [playwright-go](https://github.com/playwright-community/playwright-go).

**Browser drivers are downloaded automatically on first run** — no setup needed! The download happens once and is cached locally.

Optionally, pre-download drivers for offline use:
```sh
puppeteer-mcp --install-drivers
```

See [DEPLOYMENT.md](DEPLOYMENT.md) for more setup options and troubleshooting.
...
**Attach.** The server connects to an existing Chrome with a debugging port.

You can start Chrome manually:
```sh
chrome.exe --remote-debugging-port=9222 --user-data-dir=D:\chrome-debug
puppeteer-mcp --cdp http://127.0.0.1:9222
```

Or let puppeteer-mcp launch it automatically:
```sh
puppeteer-mcp --cdp http://127.0.0.1:9222 --cdp-launch-cmd "chrome.exe --remote-debugging-port=9222 --user-data-dir=D:\chrome-debug"
```
...
| `browser.cdp_launch_cmd` | `""` | Command to launch the browser for attach mode (runs before connecting over CDP). |
...
| `browser.keep_screenshots` | `0` | Keep at most this many captures, deleting the oldest. `0` never deletes. |
| `browser.max_screenshot_dir_bytes` | `0` | Same, by total size of the directory. `0` never deletes. |
...
### Keeping the directory from growing

Captures accumulate, and nothing is deleted unless you ask. `browser.keep_screenshots` caps the number of files and `browser.max_screenshot_dir_bytes` caps their total size; after each capture the oldest `.png` files in the directory are deleted until both limits hold. Either can be set alone, both default to `0`, and `0` means no limit - these are your files, and a server that quietly removes them is a bad surprise. The capture just written is never deleted, even if it alone is over the size cap, and nothing that is not a `.png` directly in the directory is ever touched.
